...
ARQUIVO_LOG = 'portaria_log.json'  # Define o nome do arquivo de log a ser analisado.
...
def carregar_dados(caminho_arquivo):
    ...
    try:
        df = pd.read_csv(caminho_arquivo)  # Tenta ler o arquivo CSV para um DataFrame.
        return df  # Retorna o DataFrame se a leitura for bem-sucedida.
    except FileNotFoundError:
        raise FileNotFoundError(f"Arquivo '{caminho_arquivo}' não encontrado.")  # Lança uma exceção se o arquivo não for encontrado.
    except pd.errors.EmptyDataError:
        raise ValueError(f"Arquivo '{caminho_arquivo}' está vazio.")  # Lança uma exceção se o arquivo estiver vazio.
    except pd.errors.ParserError:
        raise ValueError(f"Erro ao analisar o arquivo '{caminho_arquivo}'. Verifique o formato.")  # Lança uma exceção se houver um erro ao analisar o arquivo.
...
def analisar_logs_acesso(caminho_arquivo=ARQUIVO_LOG):
    ...
    try:
        df = carregar_dados(caminho_arquivo)  # Carrega os dados do arquivo de log.
